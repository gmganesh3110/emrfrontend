...
const EditResults: React.FC<EditVitalProps> = ({ editId, closeModals }) => {
  ...
  const fetchVital = async () => {
    try {
      const response: any = await getaxios(
        `http://localhost:3000/results/${editId}`
      );
      if (response) {
  ...
  const onFinish = async (values: any) => {
    try {
      const res = await putaxios(`http://localhost:3000/results/${editId}`, {
        ...values,
        modifiedBy: User.id,
      });
      if (res) {
